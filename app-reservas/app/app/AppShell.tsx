...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  if (!hydrated) {
    return (
      <div className="min-h-screen overflow-x-hidden bg-gradient-to-br from-[#4C5372] via-[#7C7E9D] to-[#949AB1] p-2 sm:p-4 md:p-6">
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-gradient-to-br from-[#FFFDF6] via-[#E2D4E0] to-[#949AB1] p-2 sm:p-4 md:p-6">
      <div className="grid min-h-[calc(100vh-1rem)] w-full gap-4 rounded-[1.5rem] border border-[#E2D4E0] bg-[#FFFDF6]/90 p-2 shadow-xl sm:min-h-[calc(100vh-2rem)] sm:p-4 md:grid-cols-[260px_minmax(0,1fr)] md:gap-5 md:rounded-[2rem] md:p-5">
        <aside className={`hidden rounded-3xl bg-gradient-to-b ${sidebarTheme} p-4 text-white shadow-xl md:block`}>
          ...
          <button
            ...
            className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/10 px-3 py-2.5 text-sm font-medium text-white transition hover:bg-red-500/25"
          >
        ...
        <section className="min-w-0 overflow-x-hidden rounded-2xl bg-[#FFFDF6] p-3 shadow-lg sm:rounded-3xl sm:p-5 md:p-6">
          <header className="mb-6 flex flex-col gap-3 rounded-2xl border border-[#E2D4E0] bg-[#FFFDF6] p-3 shadow-sm sm:p-4 md:flex-row md:items-center md:justify-between">
            <div>
              <div className="mb-1 flex items-center gap-2 md:hidden">
                <button
                  type="button"
                  onClick={() => setMobileMenuOpen(true)}
                  className="rounded-lg border border-[#949AB1] bg-white p-2 text-[#4C5372]"
              ...
              <p className="hidden text-xs font-semibold uppercase tracking-[0.2em] text-[#949AB1] md:block">Dashboard</p>
              <h1 className="text-xl font-bold text-slate-900">Bienvenido, {customer?.fullName || "Usuario"}</h1>
            </div>

            <div className="flex flex-wrap items-center gap-3">
              <label className="hidden items-center gap-2 rounded-xl border border-[#E2D4E0] bg-white px-3 py-2 text-xs text-[#949AB1] lg:flex">
                <span>Buscar</span>
                <input
                  type="text"
                  placeholder="Buscar en tu panel"
                  className="w-40 bg-transparent text-sm text-[#4C5372] outline-none placeholder:text-slate-400"
                />
              </label>

              <div className="flex max-w-full items-center gap-2 rounded-xl border border-[#E2D4E0] bg-white px-3 py-2">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#7C7E9D] to-[#4C5372] text-xs font-semibold text-white">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-[#4C5372]">{roleBadge}</div>
                </div>
              </div>
            </div>
          </header>
